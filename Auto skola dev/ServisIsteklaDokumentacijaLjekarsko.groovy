...
String query = "issuetype = Kandidat AND cf[10417] <= startOfDay()" //AND cf[10417] <= startOfDay() LJEKARSKO
def queryIssue = shell.parse(new File("$basePath/general.groovy"))
def issuesLj = queryIssue.executeJQL(query)
log.debug(issuesLj)

for (i = issuesLj.size() - 1; i >= 0; i--) {
    MutableIssue istekliKandidat = ComponentAccessor.getIssueManager().getIssueByKeyIgnoreCase(issuesLj[i].key)
