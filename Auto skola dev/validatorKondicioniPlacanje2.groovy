...
//Issue issue = issueManager.getIssueByKeyIgnoreCase("KAN-4086")
...
def posaljiZahtjev = issue.getCustomFieldValue(gratisField).toString()
    ...
    def ugovorenIznos = issue.getCustomFieldValue(ugovorenIznosField).toDouble()
    def zaPlatiti = issue.getCustomFieldValue(zaPlatitiField).toDouble()
	cijenaCasa = issue.getCustomFieldValue(ugovorenaCijenaField).toDouble() / issue.getCustomFieldValue(zeljenoTrajanjeField).toDouble() * issue.getCustomFieldValue(zakazanoTrajanjeField).toDouble()
    log.debug(cijenaCasa)
